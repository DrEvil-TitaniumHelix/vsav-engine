...
a schema-3 (phase-3 flow) game, and TurnStepper /
...
def fresh(seed, phase4=None):
    tmp = tempfile.mkdtemp(prefix="aus_ai_")
    return NapoleonicGame(G, SCEN, tmp, seed=seed, phase4=phase4), tmp


g0 = NapoleonicGame(G, SCEN, tempfile.mkdtemp(), seed=0)
check(not hasattr(g0, "tier") and G.spec.get("policy_ai"),
      "gate is tier-free; policy_ai declared in game.json (seat model)")

tot = {"fire": 0, "shock": 0, "windows": 0, "rej": 0}
for seed in SEEDS:
    t0 = time.time()
    g, tmp = fresh(seed)
    check(g.s["schema"] == 4 and g._p4,
          f"seed {seed}: game runs the full schema-4 gate")
...
# ------------------------------------------------ schema-3 flow (old-log replay path)
g1, tmp1 = fresh(5, phase4=False)
check(g1.s["schema"] == 3,
      "phase4=False still pins the phase-3 flow (schema 3)")
turns, log = ai.play_game(g1, max_turns=2)
check(not any(e.get("error") for e in log),
      f"schema-3 game runs without stalls ({len(log)} actions to "
      f"turn {g1.s['turn']})")
okv, msg = verify_game.verify(
    HERE, os.path.join(tmp1, "game_austerlitz-gmt.log.jsonl"))
check(okv, f"schema-3 replay: {msg}")

g2, tmp2 = fresh(5)
check(g2.s["schema"] == 4, "default = the schema-4 gate")
