...
<style>
  ...
  #topmenu { position:absolute; top:22px; right:40px; }
  #menubtn { background:#23262c; color:#dde3ea; border:1px solid #3a3f47; border-radius:8px;
             padding:8px 16px; font-size:14px; cursor:pointer; }
  #menubtn:hover { border-color:#3a6ea5; }
  #menudrop { display:none; position:absolute; right:0; top:42px; min-width:280px;
              background:#23262c; border:1px solid #3a3f47; border-radius:10px;
              box-shadow:0 8px 26px rgba(0,0,0,.55); overflow:hidden; z-index:10; }
  #menudrop a { display:block; padding:12px 16px; color:#dde3ea; font-size:13.5px;
                text-decoration:none; border-bottom:1px solid #2a2d33; }
  #menudrop a:last-child { border-bottom:0; }
  #menudrop a:hover { background:#272b31; color:#9cc4ee; }
  #menudrop a .d { display:block; color:#8a8f98; font-size:11.5px; margin-top:2px; }
</style>
</head>
<body>
<div id="topmenu">
  ...
  <div id="menudrop">
    <a href="screen/">Module screener<span class="d">search the VASSAL library — can your game be encoded?</span></a>
    <a href="https://github.com/DrEvil-TitaniumHelix/vsav-engine/issues" target="_blank" rel="noopener">Report a bug<span class="d">or request an encoding — GitHub issues</span></a>
    <a href="https://github.com/DrEvil-TitaniumHelix/vsav-engine" target="_blank" rel="noopener">Engine source<span class="d">the open engine repo (bring-your-own modules)</span></a>
  </div>
</div>
<header>
...
document.addEventListener('click', () => { md.style.display = 'none'; });
</script>
""" + BEACON + """
</body>
</html>
...
def main():
    ...
    # library census and COMMITTED - clean clones and CI never see the census
    idx = os.path.join(ROOT, "web", "screener", "index.json")
    if not os.path.isfile(idx):
        raise SystemExit("web/screener/index.json missing - run "
                         "tools/build_screen_index.py (needs the local census)")
    os.makedirs(os.path.join(OUT, "screen"))
    scr = open(os.path.join(ROOT, "web", "screener.html"), encoding="utf-8").read()
    scr = scr.replace("</body>", BEACON + "\n</body>")
    open(os.path.join(OUT, "screen", "index.html"), "w", encoding="utf-8").write(scr)
    shutil.copy(idx, os.path.join(OUT, "screen", "index.json"))
    print(f"screen: {os.path.getsize(idx)//1024} KB library index")
