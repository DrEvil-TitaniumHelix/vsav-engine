...
def build_scenario(oob, mod, tr):
    ...
    return {
        ...
        "rules_scope": {
            "status": "ENCODING IN PROGRESS - bites 1-3 and 6 of 7 enforced by engine/naw.py (movement, ZOC, exit, combat arithmetic + per-attack legality, reinforcement, ledgers, victory, demoralization); the battle action, the mandatory-attack assignment and result application (bites 4-5) wait on rulings OR-5/OR-6/OR-16. Not playable until every COVERAGE_MATRIX.md cell is ENFORCED or UNREACHABLE-with-evidence.",
            "enforced": [
                ...
                "Through friendly units yes, through or into enemy units never; a unit may end its own move on a friendly unit mid-phase only if that unit can still move off (unmoved, not in enemy ZOC), and the Movement Phase cannot close while any hex holds two units [MOV-07/MOV-08/MOV-09 - reading A per SPI 1979 4.4 'never end a Movement Phase stacked', NAW2-OR-2]",
                ...
                "Loss ledgers and victory: printed Combat Strength destroyed per side, Prussian losses Allied, exited units not losses; Allied win on the fortieth French point first; French win on the fortieth Allied point first + seven exits (before or after); both-at-once (EX) -> French if seven exited else Allied; checked the instant it happens; draw at the end of Game-Turn 10 [VIC-01..07, VIC-11, VIC-14, VIC-05/06, REI-05]",
                "Allied demoralization: latched the instant the fortieth Allied point falls with fewer than seven exits, permanent, Allied attacks -1 column / French +1 (clamped at the printed ends), a later fortieth French point changes nothing [DEM-01..09; NAW2-OR-19 A]",
            ],
            "not_yet_enforced": [
                "The battle ACTION itself (declare, roll, apply) and the phase-level mandatory-attack assignment (CBT-06/07/10 - needs rulings NAW2-OR-5/OR-6), result application: retreat (victor chooses), disruption, EX, advance, artillery immunity (bites 4-5; OR-16 pending; the rest answered by SPI 1979 - see RULINGS_FOR_BRUCE.md)",
            ],
            "rulings": [
                ...
                "NAW2-OR-6 (Bruce 2026-08-17): mandatory-attack obligations are fixed at the start of the Combat Phase, resolved on the live board, and lapse when their contact disappears; OR-5 closes by proof (a complete assignment always exists at phase start); OR-16 A (an advanced unit leaves the list)",
                "MOV-09 stacking under reading A (SPI 1979 4.4): a unit may end its own move on a friend mid-phase, the phase cannot close while any hex holds two units; wedge guard - only onto a friend that can still move off",
