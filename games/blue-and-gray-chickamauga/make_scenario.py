...
def mkunit(u, side, hexnum=None, due=None, entry=None):
    ...
    base = sn[:-2] if sn.endswith(" c") else sn
    e = {"id": slug(base), "slot": sn, "side": side, "img": img,
...
wilder = next(u for u in units if u["id"] == "wilder")
assert wilder["hex"] == [10, 22], "Wilder must sit at chart hex 1022, not module 0822"
assert any(u["id"] == "2-4-xiv" and u["hex"] == [8, 22] for u in units), "2/4/XIV at 0822"
